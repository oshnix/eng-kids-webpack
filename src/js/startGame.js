...
export const initGame = (data) => {
  const playNextWord = () => {
    ...
    playAudio(require("../" + data[randomIndex].audioSrc));
  };
  ...
  const finishGame = () => {
    ...
    if (!errors) {
      img.setAttribute("src", require("../img/excellent.png"));
      img.setAttribute("alt", "excellent");

      playAudio(require("../audio/success.mp3"));
    } else {
      img.setAttribute("src", require("../img/fail.png"));
      img.setAttribute("alt", "fail");

      playAudio(require("../audio/failure.mp3"));
  ...
  cardList.querySelectorAll(".cards-list__card").forEach((cardElement) => {
    cardElement.addEventListener("click", () => {
      ...
      playAudio(require(`../audio/${isCorrect ? "correct" : "error"}.mp3`));
